  ...
  <header class="">
    <div class="container m-auto flex flex-wrap items-center justify-end">
      <div class="flex-1 flex items-center">
        ...
        <a href="/" class="uppercase text-sm font-mono pl-4 font-semibold no-underline text-indigo-dark hover:text-indigo-darker">Notes</a>
      </div>
      <div>
        <router-link to="/" class="" v-if="!signedIn()">Sign in</router-link>
        <router-link to="/signup" class="" v-if="!signedIn()">Sign Up</router-link>
        <router-link to="/notes" class="" v-if="signedIn()">Notes</router-link>
  ...
  methods: {
    ...
    signOut () {
        ...
        $.ajax({
            ...
            error(error) {
                context.setError(error, 'Cannot sign out')
            }
        })
